...
class MyAction extends AbstractAction {
  ...
  public static KeyStroke parseAccel(String s) {
    ...
    KeyStroke k = null;
    if (accelKeyCode != 0) {
      k = KeyStroke.getKeyStroke(accelKeyCode, (ctrlFlag ? modifierMask(0) : 0)
          | (altFlag ? modifierMask(1) : 0) | (shiftFlag ? modifierMask(3) : 0));
    }
    if (ds) {
      System.out.println(
          " code=" + accelKeyCode + " s=" + shiftFlag + " a=" + altFlag + " c=" + ctrlFlag + "\n k=" + k);
    }
    return k;
  }

  /**
   * Return string representation of keystroke, for display in tooltip
   * 
   * @param k
   *          KeyStroke
   * @return String
  ...
  public static String toString(KeyStroke k) {
    ...
    final int[] mf = { ActionEvent.CTRL_MASK, ActionEvent.ALT_MASK, ActionEvent.SHIFT_MASK,
        ActionEvent.META_MASK, };
    final String[] mn = { "Ctrl", "Alt", "Shift", "Meta", };
   ...
   * 
   * @param s
   *          : string, it is wrapped in <html>...</html> tags
   */
